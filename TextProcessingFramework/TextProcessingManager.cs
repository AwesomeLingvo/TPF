...
namespace TextProcessingFramework
{
    public class TextProcessingManager
    {
        ...
        public void Register(IEnumerable<BaseTextProcessor> collectionOfProcessors)
        {
            ...
            while (registrationQueue.Count > 0)
            {
                for (int i = 0; i < previousCount; ++i)
                {
                    ...
                    RegisterState result = this.RegisterProcessor(processor);
                    if (result == RegisterState.OutputLayerNotUniq)
                        throw new InvalidOperationException("Name of output layer is not unique. Layer may be generated by one processor only!");
                    if (result != RegisterState.InputLayerNotFound)
                        registrationQueue.Enqueue(processor);
                }
                if (previousCount == registrationQueue.Count)
                {
                    throw new InvalidOperationException("Collection of processors contains insoluble depending!");
                }
        ...
        /// <summary>
        /// Get a copy of registered layer's name list. 
        /// </summary>
